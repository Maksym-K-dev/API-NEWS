<?php
if ( ! class_exists( 'News_Post_Social_Widget' ) ) {
	/**
	 * Adds News Post Social Widget.
	 */
	class News_Post_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$news_post_social_widget = array(
				'classname'   => 'news-post-widget social-widget style-2',
				'description' => __( 'Retrive Social Widget', 'news-post' ),
			);
			parent::__construct(
				'news_post_social_widget',
				__( 'Artify Widget: Social Widget', 'news-post' ),
				$news_post_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Link %d :', 'news-post' ), $i ); ?></label>
